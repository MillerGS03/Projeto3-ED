...
public class Arvore<Dado> where Dado : IComparable<Dado>
{
    ...
    public void Rearranja(NoArvore<Dado> aux, ref NoArvore<Dado> atualAnt)
    {
        if (aux.Dir != null)
            Rearranja(aux.Dir, ref atualAnt); // Procura Maior
        else
    ...
    private void Excluir(NoArvore<Dado> atual, Dado procurado)
    {
        NoArvore<Dado> atualAnt;
        if (atual == null)
            Console.WriteLine("Não Existe a chave procurada!");
        else if (atual.Info.CompareTo(procurado) > 0)
            Excluir(atual.Esq, procurado);
        else if (atual.Info.CompareTo(procurado) < 0)
            Excluir(atual.Dir, procurado);
        else
        {
            ...
            { // pai de 2 filhos
                Rearranja(atual.Esq, ref atualAnt);
